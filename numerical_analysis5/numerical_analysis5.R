...
if(glob_make_statplots) {  
  ...
  print(plot_grid(p1 + guides(shape = "none", color = "none"),
                  p2 + guides(shape = "none", color = "none"), 
                  p3, 
...
#Set below 0 values to 0
# (all runs already end at the same time)
ybig2 <- mutate(ybig2,
                Density = ifelse(Density < 0, 0, Density))
...
#Calculate phage-free values analytically
# (a holdover from how plots was originally made I can't be bothered to fix)
...
#Reshape phage-free numerically-calculated values 
#Change/duplicate rows where a_S1 == 0 to be the 5 a_S1 vals
ysum2 <- dup_rows(ysum2, mut_col = "a_S1", from_vals = 0, 
                   to_vals = 10**c(-8:-12))

#Relative auc
ysum2 <- mutate(group_by(ysum2, u_S1, u_S2, k, z, d, h,
...
ybig2_B <- interp_data(df = ybig2_B,
                        x = "time", y = "Density",
                        subset_by = ybig2_B$uniq_run)
ybig2_B <- mutate(group_by(ybig2_B, u_S1, k, h, time),
                   Dens_norm = Density - logis_func(S_0 = init_S1,
                                                    u_S = u_S1,
                                                    k = k,
                                                    times = time))
ybig2_B <- filter(ybig2_B, time %% 20 == 0)

ybig2_B_wide <- tidyr::pivot_wider(ybig2_B,
                                    names_from = time,
                                    names_prefix = "t_",
                                    values_from = c(Density, Dens_norm))

mypca <- prcomp(ybig2_B_wide[, grep("Density_", colnames(ybig2_B_wide))[-1]],
                center = TRUE, scale = TRUE, retx = TRUE)
mypcanorm <- prcomp(ybig2_B_wide[, grep("Dens_norm", colnames(ybig2_B_wide))[-1]],
                    center = TRUE, scale = TRUE, retx = TRUE)
...
if(glob_make_statplots) {
  ...
  png("./statplots/extrafigure_run2_auccurves.png", width = 6, height = 4,
      units = "in", res = 300)
  print(cowplot::plot_grid(
  ...
  ggplot(data = filter(ysum2, init_moi == 0.01),
         aes(x = log10(a_S1), y = log10(auc),
             color = as.factor(k))) +
    geom_point() +
    geom_line(aes(group = paste(u_S1, k)))
  ggplot(data = filter(ysum2, h == 0, init_moi == 0.01),
  ...
  png("./statplots/extrafigure_run2_relauc_controlauc_all.png", 
      width = 4.5, height = 6,
      units = "in", res = 300)
  print(
    cowplot::plot_grid(
  ...
  png("./statplots/extrafigure_run2_maxtime_contour_all.png", width = 6.2, height = 6,
      units = "in", res = 300)
  print(cowplot::plot_grid(
    cowplot::plot_grid(p1 + theme(legend.position = "none"), 
                       p2 + theme(legend.position = "none"), 
  ...
  f8d <- ggplot(data = ysum3,
    ...
    theme(axis.title = element_text(size = 22),
          ...
          legend.title = element_text(
            size = 20, margin = margin(0, 0, 0.07, 0, unit = "npc")),
          legend.text = element_text(size = 18)) +
    NULL
...
if (glob_make_statplots) {
  ...
  p2 <- ggplot(data = filter(ysum7, init_P > 10**3.9, init_P < 10**4.1),
               aes(color = as.factor(log10(a_S1)), x = log10(init_S1),
                   y = peak_time/60)) +
    geom_point(aes(shape = extin_flag),
               size = 3) +
    geom_line() +
  ...
  p1 <- ggplot(data = filter(ysum7, init_P > 10**3.9, init_P < 10**4.1), 
               aes(x = log10(a_S1), y = log10(init_S1))) +
    geom_contour_filled(aes(z = peak_time/60), alpha = 0.5) +
    geom_point(aes(color = peak_time/60, shape = extin_flag),
               size = 3) +
    scale_color_viridis_c(name = "Time of Peak\nBacterial\nDensity (hr)",
                          breaks = c(3, 6, 9, 12)) +
    scale_shape_manual(breaks = c("neark", "noextin", "none"), 
                       values = c(4, 4, 16)) +
    scale_y_continuous(labels = math_format(10^.x)) +
  ...
  p3 <- ggplot(data = filter(ysum7, init_P > 10**3.9, init_P < 10**4.1), 
               aes(x = log10(a_S1), y = log10(init_S1))) +
  ...
  p4 <- ggplot(data = filter(ysum7, init_S1 == 10**6), 
               aes(x = log10(a_S1), y = log10(init_P))) +
    geom_contour_filled(aes(z = peak_dens), alpha = 0.5) +
    # geom_segment(aes(x = a_S1, xend = a_S1,
  ...
  p5 <- ggplot(data = filter(ysum7, init_P > 10**3.9, init_P < 10**4.1), 
               aes(x = log10(a_S1), y = log10(init_S1))) +
    geom_contour_filled(aes(z = log10(extin_time_4/60)), alpha = 0.5) +
    # geom_segment(aes(x = a_S1, xend = a_S1,
    #                  y = init_S1*qpois(0.025, 10)/10,
  ...
  p6 <- ggplot(data = filter(ysum7, init_S1 == 10**6), 
               aes(x = log10(a_S1), y = log10(init_P))) +
    geom_contour_filled(aes(z = log10(extin_time_4/60)), alpha = 0.5) +
    # geom_segment(aes(x = a_S1, xend = a_S1,
    #                  y = init_P*qpois(0.025, 10)/10,
  ...
  p7 <- ggplot(data = filter(ysum7, init_P > 10**3.9, init_P < 10**4.1), 
               aes(x = log10(a_S1), y = log10(init_S1))) +
    geom_contour_filled(aes(z = log10(auc/60)), alpha = 0.5) +
    # geom_segment(aes(x = a_S1, xend = a_S1,
    #                  y = init_S1*qpois(0.025, 10)/10,
  ...
  p8 <- ggplot(data = filter(ysum7, init_S1 == 10**6), 
               aes(x = log10(a_S1), y = log10(init_P))) +
    geom_contour_filled(aes(z = log10(auc/60)), alpha = 0.5) +
    # geom_segment(aes(x = a_S1, xend = a_S1,
    #                  y = init_P*qpois(0.025, 10)/10,
  ...
  png("./statplots/figS14_run7_othermetrics_a_initP_initS_contour.png", 
      ...
      units = "in", res = 300)
  print(cowplot::plot_grid(p1, p2, p3, p4, p5, p6, p7, p8, ncol = 2, 
  ...
  print(
    ggdraw(
      cowplot::plot_grid(
        ...
        align = "hv", axis = "tblr", label_size = 20, 
        vjust = 0.8,
        rel_heights = c(0.15, 1, 1)) +
        draw_plot(f8a_inset, 0.121, 0.61, 0.086, 0.25))
  )
  dev.off()
