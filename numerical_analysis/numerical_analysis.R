...
ybig1$deriv <- calc_deriv(y = ybig1$Density, 
                           percapita = FALSE,
                           subset_by = paste(ybig1$uniq_run, ybig1$Pop), 
                           x = ybig1$time,
                           x_scale = 60)
ybig1$deriv_percap <- calc_deriv(y = ybig1$Density, 
                                percapita = TRUE,
                                subset_by = paste(ybig1$uniq_run, ybig1$Pop), 
                                x = ybig1$time,
                                x_scale = 60)
...
if (glob_make_statplots) {
  ...
  tiff("./run2_statplots/extintime_rel_to_maxtime_tau=31.6.tiff",
       width = 6, height = 4, units = "in", res = 300)
  print(ggplot(data = y_summarized2[y_summarized2$near_k == 0 &
  ...
  tiff("./run2_statplots/extinrate_phageatmax1_tau=31.6.tiff",
       width = 6, height = 5, units = "in", res = 300)
  print(ggplot(data = y_summarized2[y_summarized2$near_k == 0  &
                                      y_summarized2$tau == 31.6, ],
               aes(x = phage_atmaxdens, y = extin_time/max_time,
  ...
  ggplot(data = y_summarized2[y_summarized2$near_k == 0, ],
         aes(y = phage_atmaxdens, x = max_time/60, 
             color = as.factor(b), shape = as.factor(a))) +
    geom_point() +
    scale_y_continuous(trans = "log10") +
    scale_x_continuous(trans = "log10") +
  ...
  ggplot(data = y_summarized2[y_summarized2$u_S == 0.0179, ],
         aes(x = a, y = tau)) +
    geom_contour_filled(aes(z = log10(phage_atmaxdens))) +
    geom_point(aes(color = log10(phage_atmaxdens)), size = 2) +
    scale_color_viridis() +
    facet_wrap(b ~ .) +
    scale_x_continuous(trans = "log10") +
    scale_y_continuous(trans = "log10")
...
y_summarized8 <- summarize(ybig8,
                           max_index = find_local_extrema(y = Density[Pop == "B"],
                                                        return_minima = FALSE,
                                                        width_limit_n = 3)[1],
                           max_dens = Density[Pop == "B"][max_index],
                           max_time = time[Pop == "B"][max_index],
                           extin_index = find_local_extrema(y = Density[Pop == "B"],
                                                            return_maxima = FALSE,
                                                            width_limit_n = 3)[1],
